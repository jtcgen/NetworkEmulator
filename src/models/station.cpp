 ...
 */
Station::Station(const char *iface, const char *rtable, const char *host, bool is_router) : is_router_(is_router), debug("Station", true) {
 ...
 */
void Station::load_hosts(const char *host) {
    std::string line;
 ...
 */
SocketData* Station::get_iface_socket(Iface* iface) {
 ...
 *  @param iface    Interface file name
 */
void Station::load_interfaces(const char *iface) {
 ...
 *  @param rtable       Routing table file name.
 */
void Station::load_routes(const char *rtable) {
...
void Station::send_ether_pkt(IPAddr src_ip, IPAddr dst_ip, std::string data) {
    ...
    if (arp_.has_mapping(next_hop.ip_addr)) {
        ...
        if (is_router_) {
            pp = { next_hop.ip_addr, dst_ip, src_ip, next_hop.iface, data };
        } else {
            pp = { next_hop.ip_addr, dst_ip, iface.first->ip_addr, next_hop.iface, data };
        }
        pq_.push_back(pp);
    ...
    // Send Ether Packet
    if (write(iface.second->fd, &ether_pkt, sizeof(EtherPkt)) < 0)
        std::cout << "Could not send Ethernet Packet\n";
}

void Station::send_ether_pkt(IPAddr dst_ip, std::string data) {
    send_ether_pkt("", dst_ip, data);
}
 ...
 */
void Station::send_cmd(std::string dst, std::string msg) {
 ...
 */
void Station::parse_address(IPAddr addr, int *arr) {
    size_t pos = 0;
    std::string delim = ".";
    std::string tok;
 ...
 */
NextHop Station::get_next_hop(IPAddr host_dst) {
 ...
 */
IfaceSocketPair Station::get_iface(std::string name) {
 ...
 */
std::string Station::get_iface_for_ip(IPAddr ip) {
    std::string host;
    for (auto h : hosts_) {
 ...
 */
void Station::menu_commands(char *cmd, bool &quit) {
    // Eliminate new line
    std::string buff(cmd);
 ...
 */
void Station::print_ether_pkt(const EtherPkt &pkt) {
 ...
 */
void Station::socket_to_bridge(SocketData *&sock_data) {
 ...
 */
bool Station::is_intended_ip_destination(IPAddr ip) {
    for (auto i : ifaces_) {
...
void Station::start() {
    ...
    int *flags = new int[ifaces_.size()];
    
    // Connect to bridges
    for (unsigned int i = 0; i < ifaces_.size(); ++i) {
        FD_SET(ifaces_[i].second->fd, &read_set);
        max_fd = ifaces_[i].second->fd;
        
        // Set non-blocking flag for bridge descriptors
        flags[i] = fcntl(ifaces_[i].second->fd, F_GETFL, 0);
        fcntl(ifaces_[i].second->fd, F_SETFL, flags[i] | O_NONBLOCK);
            ...
            if (read(ifaces_[i].second->fd, msg, MSG_SIZE) > 0) {
                if (strcmp(msg, "Accepted") == 0) {
                    // Reset to blocking on socket file descriptor
                     fcntl(ifaces_[i].second->fd, F_SETFL, flags[i]);
                    accepted = true;
                    break;
                }
            } else sleep(2);
        ...
        if (!accepted) {
            ...
            delete ifaces_[i].first; delete ifaces_[i].second;
        ...
        if (FD_ISSET(0, &read_set)) {
            ...
            for (auto i : ifaces_) {
                if (FD_ISSET(i.second->fd, &read_set)) {
                    size_t n;
                    if ((n = read(i.second->fd, &recv_ether_pkt, sizeof(recv_ether_pkt))) == 0) {
                        quit = true;
                        ...
                        if (recv_ether_pkt.type == 0) {      /*  ARP PACKET  */
                            if (recv_ether_pkt.arp_pkt.op == 0) {    /*  REQUEST  */
                                ...
                                for (auto i : ifaces_) {
                                    debug.get_oss() << "comparing " << recv_ether_pkt.arp_pkt.dst_ip << " with " << i.first->ip_addr;   debug.print();
...
//                                    send_ether_pkt(recv_ether_pkt.ip_pkt.dst_ip, recv_ether_pkt.ip_pkt.data);
                                    send_ether_pkt(recv_ether_pkt.ip_pkt.src_ip, recv_ether_pkt.ip_pkt.dst_ip, recv_ether_pkt.ip_pkt.data);
                                }
                            }
                        }
                    }
